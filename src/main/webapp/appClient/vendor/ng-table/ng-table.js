...
//	$templateCache.put('ng-table/filters/date.html', '<input type="date" name="{{name}}" ng-disabled="$filterRow.disabled" ng-model="params.filter()[name]" class="input-filter form-control" maxlength="10" placeholder="aaaa-mm-jj" />');
	$templateCache.put('ng-table/filters/date.html', '<p class="input-group"><input type="date" name="{{name}}" ng-disabled="$filterRow.disabled" ng-model="params.filter()[name]" class="form-control" uib-datepicker-popup ng-model="dt" is-open="popup.opened" datepicker-options="dateOptions" close-text="Fermer" current-text="Aujourd\'hui" clear-text="Effacer" popup-placement="auto bottom-left" /><span class="input-group-btn"><button type="button" class="btn btn-default" ng-click="popup.opened=true"><i class="glyphicon glyphicon-calendar"></i></button></span></p>');
