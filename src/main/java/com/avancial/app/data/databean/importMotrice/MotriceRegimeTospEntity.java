...
@NamedQueries({ @NamedQuery(name = "MotriceRegimeTosp.getAll", query = "SELECT t FROM MotriceRegimeTospEntity t"),
      ...
      @NamedQuery(name = "MotriceRegimeTosp.deleteByRegimes", query = "DELETE FROM MotriceRegimeTospEntity t WHERE t.motriceRegime IN (:regimes)"),
      @NamedQuery(name = "MotriceRegimeTospEntity.getLastId", query = "SELECT MAX( t.idMotriceRegimeTosp ) FROM MotriceRegimeTospEntity t") })
