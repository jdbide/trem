...
public class TraiteMotriceRegimeDistribution implements ITraiteMotriceRegime {
   ...
   public void traite(MotriceTrainTrancheEntity motriceTrainTrancheEntity, MapIdTablesMotriceRegime mapIdTablesMotriceRegime, MapGeneratorTablesMotriceRegime mapGeneratorTablesMotriceRegime, EntityManager entityManager) {
      ...
      Query queryRDistribution = entityManager
            .createNativeQuery("SELECT distrib.DTRC_CODE AS distribIndexMotriceRegimeDistribution, distrib.DTRC_REGI AS motriceRegime " + "FROM tremas_import_tmddtrc AS distrib " + "INNER JOIN tremas_import_tmdcath AS cath ON distrib.DTRC_TRCH_COD_CIE = cath.CATH_TRCH_COD_CIE "
                  + "AND distrib.DTRC_TRCH_NUM_TRA1 = cath.CATH_TRCH_NUM_TRA1 " + "AND distrib.DTRC_TRCH_IND_FER = cath.CATH_TRCH_IND_FER " + "AND distrib.DTRC_TRCH_NUM = cath.CATH_TRCH_NUM " + "WHERE cath.CATH_SSIM = ? " + "AND distrib.DTRC_TRCH_NUM_TRA1 = ? " + "ORDER BY motriceRegime");
