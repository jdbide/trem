...
public class TraiteMotriceRegimeSpecificity extends AFiltreObjetMetier implements ITraiteMotriceRegime {
   ...
   public void traite(MotriceTrainTrancheEntity motriceTrainTrancheEntity, MapIdTablesMotriceRegime mapIdTablesMotriceRegime,
      ...
      Query query = entityManager.createNativeQuery("SELECT voiture.VOIT_NUM_RESA AS coachNumberMotriceRegimeSpecificity, "
            ...
            + "sppl.SPPL_SPEC_COD AS stateCodeMotriceRegimeSpecificity, " + "sppl.SPPL_REGI AS regime " + "FROM tremas_import_tmdvoit AS voiture "
            + "INNER JOIN tremas_import_tmdcath AS cara ON voiture.VOIT_TRCH_COD_CIE = cara.CATH_TRCH_COD_CIE "
            + "AND voiture.VOIT_TRCH_NUM_TRA1 = cara.CATH_TRCH_NUM_TRA1 " + "AND voiture.VOIT_TRCH_IND_FER = cara.CATH_TRCH_IND_FER "
            + "AND voiture.VOIT_TRCH_NUM = cara.CATH_TRCH_NUM " + "INNER JOIN tremas_import_tmdsppl AS sppl ON voiture.VOIT_NUM = sppl.SPPL_VOIT_NUM "
            + "AND voiture.VOIT_TRCH_COD_CIE = sppl.SPPL_VOIT_COD_CIE " + "AND voiture.VOIT_TRCH_NUM_TRA1 = sppl.SPPL_VOIT_NUM_TRA1 "
            + "AND voiture.VOIT_TRCH_NUM = sppl.SPPL_VOIT_TRCH_NUM " + "AND voiture.VOIT_TRCH_IND_FER = sppl.SPPL_VOIT_IND_FER "
            + "WHERE cara.CATH_SSIM = ? " + "AND cara.CATH_TRCH_NUM_TRA1 = ? AND cara.CATH_ETAT_TRCH = ? " + "AND sppl.SPPL_SPEC_COD = 'F' "
      ...
      query = entityManager.createNativeQuery("SELECT voiture.VOIT_NUM_RESA AS coachNumberMotriceRegimeSpecificity, "
            + "LPAD(spco.SPCO_COMP_NUM, 3, '0') AS compartmentNumberMotriceRegimeSpecificity, " + "'' AS seatNumberMotriceRegimeSpecificity, "
            + "spco.SPCO_SPEC_COD AS stateCodeMotriceRegimeSpecificity, " + "spco.SPCO_REGI AS regime, " + "tyvo.TYVO_DIAV_COD "
            ...
            + "AND voiture.VOIT_TRCH_COD_CIE = spco.SPCO_VOIT_COD_CIE " + "AND voiture.VOIT_TRCH_NUM_TRA1 = spco.SPCO_VOIT_NUM_TRA1 "
            + "AND voiture.VOIT_TRCH_NUM = spco.SPCO_VOIT_TRCH_NUM " + "AND voiture.VOIT_TRCH_IND_FER = spco.SPCO_VOIT_IND_FER "
      ...
      query = entityManager
            .createNativeQuery("SELECT pcdd.PCDD_COMP_COD_DIAG AS codeDiag, COUNT( DISTINCT pcdd.PCDD_COMP_NUM_COMP ) AS nombreCompartiments "
      ...
      for (KeyMotriceRegimeSpecificity mapKey : mapRegimeCodeDiag.keySet()) {
         ...
         mapGeneratorTablesMotriceRegime.get(MotriceRegimeEntity.class).addValue(idRegime.incrementAndGet(), mapKey.getRegime(), 4,
               motriceTrainTrancheEntity.getIdMotriceTrainTranche());
