...
public class TraiteMotriceRegimeComposition implements ITraiteMotriceRegime {
	...
	public void traite(MotriceTrainTrancheEntity motriceTrainTrancheEntity,
		...
		Query queryRCompo = entityManager.createNativeQuery(
				"SELECT classe.RCTH_CLBA_COD AS classCodeMotriceRegimeComposition, "
				...
				+ "voiture.VOIT_REGI_UTIL AS periodMotriceRegime "
		...
		for (Object[] compo : listeCompo) {
			if(!oldRegime.equals(compo[5])) {// si le régime traité est différent du précédent
												// on insère une nouvelle entrée
				mapGeneratorTablesMotriceRegime.get(MotriceRegimeEntity.class).addValue(idRegime.incrementAndGet(), compo[5], 11, motriceTrainTrancheEntity.getIdMotriceTrainTranche());
			}
			// insertion du régime compo lié au régime
			mapGeneratorTablesMotriceRegime.get(MotriceRegimeCompositionEntity.class).addValue(idRegimeCompo.getAndIncrement(), compo[0], compo[1], compo[2], compo[3], idRegime.get());	
			oldRegime = (String) compo[5];
			
			// insertion des numéros de voiture liés à la compo
			if(!oldCoachNumber.equals(compo[4])) {
				mapGeneratorTablesMotriceRegime.get(MotriceRegimeCompositionCoachEntity.class).addValue(idRegimeCompoCoach.getAndIncrement(), compo[4], idRegimeCompo.get());
