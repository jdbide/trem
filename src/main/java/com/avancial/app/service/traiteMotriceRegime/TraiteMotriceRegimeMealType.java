...
public class TraiteMotriceRegimeMealType implements ITraiteMotriceRegime {
	...
	public void traite(MotriceTrainTrancheEntity motriceTrainTrancheEntity,
		...
		IMultipleInsertRequestGenerator generatorMeal = mapGeneratorTablesMotriceRegime
				.get(MotriceRegimeMealTypeEntity.class);
		...
		List<ASousRegimeTranche> listeMeal = (List<ASousRegimeTranche>) atomicTranche.get()
				.getAttributsField(Repas.class);
		if (listeMeal == null) {
		...
		for (Object[] record : rDistribution) {
			...
			generatorMeal.addValue(idMeal.getAndIncrement(), (String) record[0], (String) record[1], (String) record[2],
					idRegime);
			try {
