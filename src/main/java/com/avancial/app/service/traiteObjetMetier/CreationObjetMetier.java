...
public class CreationObjetMetier {
    ...
    public JeuDonneesPlanTransport creationPlanTransport(String environnementCompagnie, Status status,
            EntityManager entityManager, TraiteObjetMetierRegimeFactory traiteObjetMetierRegimeFactory)
            throws Exception {
        ...
        for (MotriceTrainTrancheEntity resTrainTranche : trainsTranches) {
            logger.info("Remplissage du train " + resTrainTranche.getTrainNumberMotriceTrainTranche()
                    + ", tranche " + resTrainTranche.getTrancheNumberMotriceTrainTranche() + ", statut "
                    + resTrainTranche.getTrancheStatusMotriceTrainTranche());
        ...
        /*
         * Récupération du jeu de données: pour récupérer tous les attributs, il
         * faut passer explicitement par les getters à cause du (fetch =
         * FetchType.LAZY) sur les jointures dans l'entité
        ...
        if (!trainsTranches.isEmpty()) {
            ...
            jeuDonneeEntity
                    .setDateLastUpdateJeuDonnees(trainsTranches.get(0).getJeuDonnee().getDateLastUpdateJeuDonnees());
