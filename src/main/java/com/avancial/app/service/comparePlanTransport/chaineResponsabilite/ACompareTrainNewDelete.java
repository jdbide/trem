...
/**
 * Implémentation pour les comparaisons NEW et DELETE au niveau d'un train, c'est-à-dire la détection de nouvelles tranches entre deux listes de
 * tranches.
 * 
...
public abstract class ACompareTrainNewDelete extends AChaineComparePlanTransport {
   ...
   /**
    * Compare deux listes de tranches, et retourne la liste de tranches nouvelles entre les deux.
    * 
    ...
    * @return Liste de {@link IComparaisonPlanTransport} correspondant à tous les train-tranches nouveaux dans la liste de tranches plus récentes.
    * @throws Exception
    */
   @SuppressWarnings("unchecked")
   protected MapComparaisonPlanTransport compareTrancheLists(EnumTypeComparaisonPlanTransport typeComparaisonPlanTransport, String numeroTrain,
         List<? extends IPlanTransport> tranchesAncien, List<? extends IPlanTransport> tranchesNouveau) throws Exception {
      ...
      for (Iterator<Tranche> itTrancheNouveau = (Iterator<Tranche>) tranchesNouveau.iterator(); itTrancheNouveau.hasNext();) {
         ...
         if (trancheAncien == null) {
            ...
            logger.info(
                  "Train-Tranche NEW ou DELETE : " + comparaisonPlanTransport.getNumeroTrain() + "-" + comparaisonPlanTransport.getNumeroTranche());
         ...
         /*
          * Sinon, trancheNouveau est dans ancien, et la date de fin de son régime dans ancien est inférieure strictement à celle dans nouveau
          */
            ...
            // des dates de circulation on été changées
            List<Date> newList = ListUtils.removeAll(trancheNouveau.getRegime().getListeJours(), trancheAncien.getRegime().getListeJours());
            ...
            if (!newList.isEmpty()) {
               ...
               comparaisonPlanTransport.setNumeroTranche(trancheNouveau.getNumeroTranche());
               comparaisonPlanTransport.setTrancheStatut(trancheNouveau.getTrancheStatut());
               comparaisonPlanTransport.setRegimeTranche(new Regime(trancheNouveau.getRegime().getCodeRegime(), null, null, newList));
               comparaisonPlanTransport.setTypeComparaisonPlanTransport(typeComparaisonPlanTransport);
               logger.info("Train-Tranche NEW oud DELETE (régime modifié) : " + comparaisonPlanTransport.getNumeroTrain() + "-"
                     + comparaisonPlanTransport.getNumeroTranche());
               res.putComparaison(comparaisonPlanTransport);
            }
         }
   ...
   /**
    * Cherche dans une liste une tranche égale à celle passée en paramètre, en comparant le numéro de tranche, le statut, et si possible le régime;
    * <br>
    * s'il y a une tranche exactement égale elle est retournée, sinon on retourne la dernière équivalence sur le numéro de tranche et le statut.
    * 
    * @param trancheToFind
