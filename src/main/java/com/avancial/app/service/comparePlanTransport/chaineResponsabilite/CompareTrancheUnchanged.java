...
public class CompareTrancheUnchanged extends AChaineComparePlanTransport {
   ...
   public MapComparaisonPlanTransport compare(IPlanTransport comparableAncien, IPlanTransport comparableNouveau) throws Exception {
      ...
      logger.info("Début comparaison Tranches UNCHANGED : " + trancheAncien.getNumeroTranche() + " - " + trancheNouveau.getNumeroTranche());

      /*
       * On cherche les attributs unchanged entre les deux listes, et on les enlève
       */
      /* Boucle sur les listes d'attributs de trancheNouveau */
      for (Class<?> attribut : trancheNouveau.getAttributs().keySet()) {
         logger.info("Début comparaison Attributs UNCHANGED (Tranche " + trancheAncien.getNumeroTranche() + ") : " + attribut.getSimpleName());
      ...
      /*
       * Si la liste est vide, c'est qu'aucun attribut n'est modifié : la tranche est inchangée
       */
      if (res.size() == 0) {
      ...
      logger.info("Fin comparaison Tranches UNCHANGED : " + trancheAncien.getNumeroTranche() + " - " + trancheNouveau.getNumeroTranche());
      return res;
   }
   ...
   private void removeUnchanged(List<? extends ASousRegimeTranche> sousRegimeTranchesAncien,
      ...
      if (sousRegimeTranchesAncien != null && sousRegimeTranchesNouveau != null) {
         ...
         for (Iterator<ASousRegimeTranche> itSousRegimeTrancheNouveau = (Iterator<ASousRegimeTranche>) sousRegimeTranchesNouveau
            ...
            for (Iterator<ASousRegimeTranche> itSousRegimeTrancheAncien = (Iterator<ASousRegimeTranche>) sousRegimeTranchesAncien
               ...
               /*
                * Deux attributs sont inchangés entre deux jeux de données s'ils ont le même régime et la même valeur
                */
