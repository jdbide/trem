...
/**
 * Filtre une {@link Tranche} à partir d'une valeur d'une donnée de type {@link ASousRegimeTranche} : dans la liste de valeurs de la même classe, on
 * ne retient que les données qui ont la même valeur que celle donnée au constructeur.<br>
 * Exemple : si l'on donne au constructeur un objet de classe {@link Tosp} dont la valeur du code TOSP est "01", on va retourner une tranche dont la
 * liste de {@link Tosp} a été filtrée par rapport au code "01" (on enlève tous les {@link Tosp} dont le code n'est pas "01"), mais les listes de
 * {@link CodeSat}, {@link FareProfile}... ne sont pas modifiées.
 * 
 * @author heloise.guillemaud
 *
 */
class FiltreSousRegimeTranche implements IFiltre<Tranche> {
   ...
   public Tranche filtreParCritere(Tranche object) {
      ...
      if (object.getAttributsField(this.aSousRegimeTranche.getClass()) != null) {
         ...
         for (ASousRegimeTranche aSousRegimeTranche : object.getAttributsField(this.aSousRegimeTranche.getClass())) {
            /* Si la valeur est égale à celle donnée au constructeur, on l'ajoute à la tranche résultat */
            if (this.aSousRegimeTranche.equals(aSousRegimeTranche)) {
               donnees.add(aSousRegimeTranche);
               /* On peut sortir de la boucle : il y a au plus un élément d'une valeur dans la liste */
               break;
            }
         }
