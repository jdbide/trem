...
public class TestIhmPageDataBean {
   ...
   public static WebArchive createDeployment() {
      ...
      WebArchive jar = ShrinkWrap.create(WebArchive.class).addPackage(IhmPageDataBean.class.getPackage())
            ...
            .addPackage(EntityManagerProducerSocle.class.getPackage()).addPackage(EntityManagerFactoryProviderDb2.class.getPackage()).addAsWebInfResource("WEB-INF/beans.xml", "beans.xml").addAsLibraries(lib).addAsWebInfResource("persistence.xml", "classes/META-INF/persistence.xml")
   ...
   /*
    * 
    * @Test public void testInjectionConfPersist() { // dynamic configuration Map<String, String> props = new HashMap<>(); props.put("javax.persistence.jdbc.user", "dbad_tremas"); props.put("javax.persistence.jdbc.password", "!tremas-12");
    * 
    * EntityManager mm = null; try { EntityManagerFactory emf = Persistence.createEntityManagerFactory("pu-db2", props);
    * 
    * 
    * mm = emf.createEntityManager(); } catch (Throwable ex) { ex.printStackTrace(); //throw new RuntimeException("Error al crear la factoria de JPA:->"+ ex.getMessage()); }
    * 
    * mm.getTransaction().begin(); // rubrique.setIdRubrique(1L); rubrique.setLibelleIhmRubrique("Rubrique");
    * 
    * mm.persist(rubrique);
    * 
    * // chapitre.setIdChapitre(1L); chapitre.setLibelleIhmChapitre("Chapitre"); chapitre.setIhmRubriqueTypeDataBean(rubrique); mm.persist(chapitre);
    * 
    * // page.setIdPage(1L); page.setLibelleIhmPage("Test"); page.setIhmChapitreDataBean(chapitre); mm.persist(page); mm.flush(); mm.getTransaction().commit();
    * 
    * }
    * 
    * EntityManagerProviderDb2 emDb2;
    * 
    * @Test public void testInjectionConfPersistWithEntityManagerProviderDb2() { EntityManager mm = null;
    * 
    * try { this.emDb2 = EntityManagerProviderDb2.getInstance("dbad_tremas", "!tremas-12"); mm = this.emDb2.getEm(); } catch (Throwable ex) { ex.printStackTrace(); //throw new RuntimeException("Error al crear la factoria de JPA:->"+ ex.getMessage()); }
    * 
    * mm.getTransaction().begin(); // rubrique.setIdRubrique(1L); rubrique.setLibelleIhmRubrique("Rubrique testInjectionConfPersistWithEntityManagerProviderDb2");
    * 
    * mm.persist(rubrique);
    * 
    * // chapitre.setIdChapitre(1L); chapitre.setLibelleIhmChapitre("Chapitre"); chapitre.setIhmRubriqueTypeDataBean(rubrique); mm.persist(chapitre);
