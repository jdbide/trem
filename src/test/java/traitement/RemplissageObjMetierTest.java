...
public class RemplissageObjMetierTest {
   ...
   public void testRemplissageDesserteObjMetier() throws Exception {
      ...
      Query query = this.em.createNativeQuery("SELECT DISTINCT train.trainNumberMotriceTrainTranche AS numeroTrain, train.validForRRMotriceTrainTranche AS validePourRR " + "FROM tremas_motrice_traintranche AS train ");
      ...
      for (Object[] resTrain : trains) {
         ...
         query = this.em.createNativeQuery("SELECT tranche.idMotriceTrainTranche AS idMotriceTrainTranche, tranche.trancheNumberMotriceTrainTranche, tranche.trancheStatusMotriceTrainTranche AS trancheStatut, regime.periodMotriceRegime AS regimeTranche "
               + "FROM tremas_motrice_traintranche AS tranche " + "INNER JOIN tremas_motrice_regime AS regime ON regime.idMotriceTrainTranche = tranche.idMotriceTrainTranche " + "WHERE regime.idMotriceRefRegimeType = 1 " + "AND tranche.trainNumberMotriceTrainTranche = ?");
         ...
         for (Object[] resTranche : tranches) {
            ...
            query = this.em.createNativeQuery("SELECT distrib.distribIndexMotriceRegimeDistribution AS indiceDistribution, regime.periodMotriceRegime AS regimePeriod " + "FROM tremas_motrice_regime_distribution AS distrib "
                  + "INNER JOIN tremas_motrice_regime AS regime ON regime.idMotriceRegime = distrib.idMotriceRegime " + "WHERE regime.idMotriceRefRegimeType = 10 AND regime.idMotriceTrainTranche = ?");
      ...
      for (Train train : planTransport.getTrains()) {
         for (Tranche tranche : train.getTranches()) {
            for (Distribution distribution : (List<Distribution>) tranche.getAttributsField(Distribution.class)) {
               System.out.println("(" + train.getNumeroTrain() + ", " + tranche.getNumeroTranche() + ", " + tranche.getRegime().getCodeRegime() + ", " + distribution.getIndiceDistribution() + ", " + distribution.getRegime().getCodeRegime() + ")");
            }
         }
      }
